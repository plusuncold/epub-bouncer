...
def get_correction_for_word(word : str, count : int, count_capitalized : int, dictionary, index : int, total : int):
    ...
    print(f'0 - No correction, e - end correct word, f - finish correcting words, q - quit')
    ...
    try:
        ...
        print(f'Input {user_input} is not valid. Please input \'f\', \'e\', \'q\' or a number between 0 and 9')
        return get_correction_for_word(word, count, count_capitalized, dictionary, index, total)
...
def corrections_for_words(unique_words, dict_lang):
    ...
    # Get misspelled_words by checking against dictionary, then sort by occurance (greatest to least)
    ...
    # Iterate through the words that aren't found in the dictionary getting any corrections
